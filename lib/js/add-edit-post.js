jQuery( document ).ready( function ( $ ) {
	...
	(function () {
		...
		var $checkbox = $( "#it-exchange-switch-member-access" );

		new Switchery( document.querySelector( "#it-exchange-switch-member-access" ), {
			size: 'small'
		} );

		var $settings = $( ".it-exchange-membership-content-access-rules-settings" );

		$checkbox.on( 'change', function () {
			$settings.slideToggle();

			var data = {
				action: 'it-exchange-membership-addon-toggle-content-restriction',
		...
		$.post( ajaxurl, data, function ( response ) {
			...
			if ( 0 < response.length ) {
				$( ".it-exchange-membership-restrictions" ).replaceWith( function () {
					...
					var $el = $( response );
					var dateInput = $( '.datepicker', $el );

					datePickerOpts.dateFormat = dateInput.data( 'format' );

					// Datepicker for the start and end dates.
					if ( dateInput.prop( 'type' ) != 'date' ) {
						dateInput.datepicker( datePickerOpts );
					}
	...
	metabox.on( 'change', '.it-exchange-membership-delay-rule input, .it-exchange-membership-delay-rule select', function ( e ) {
		...
		var rule_container = $( this ).closest( '.it-exchange-membership-restriction-group' );

		var ruleID = $( 'input[name="it_exchange_rule_id"]', rule_container ).val();
		var membershipID = $( 'input[name="it_exchange_membership_id"]', rule_container ).val();
		...
		updateDelayRule( postID, membershipID, ruleID, type, changes );
	} );
	 ...
	 * @param changes
	 */
	function updateDelayRule( postID, membershipID, ruleID, type, changes ) {
