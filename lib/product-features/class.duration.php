<?php
/**
 * This will control membership duration
 *
 * @since 1.0.0
...
class IT_Exchange_Addon_Membership_Product_Feature_Duration {
	 ...
	 * @return void
	*/
	function IT_Exchange_Addon_Membership_Product_Feature_Duration() {
		if ( is_admin() ) {
		...
		add_action( 'it_exchange_update_product_feature_membership-duration', array( $this, 'save_feature' ), 9, 3 );
	...
	*/
	function init_feature_metaboxes() {
		...
		if ( !empty( $post_type ) && 'it_exchange_prod' === $post_type ) {
			if ( !empty( $product_type ) &&  it_exchange_product_type_supports_feature( $product_type, 'membership-duration' ) )
	...
	*/
	function register_metabox() {
		add_meta_box( 'it-exchange-product-membership-duration', __( 'Membership Duration', 'LION' ), array( $this, 'print_metabox' ), 'it_exchange_prod', 'normal' );
	}
	...
	*/
	function print_metabox( $post ) {
		...
		$product_feature_time = it_exchange_get_product_feature( $product->ID, 'membership-duration', array( 'setting' => 'time' ) );
		$product_feature_type = it_exchange_get_product_feature( $product->ID, 'membership-duration', array( 'setting' => 'type' ) );
		
		?>
			<p class="it-exchange-duration">
				<?php _e( 'How long does this membership last?', 'LION' ); ?>
			</p>
            <input type="text" name="it-exchange-product-membership-duration-time" value="<?php esc_attr_e( $product_feature_time ); ?>" />
	...
	*/
	function save_feature_on_product_save() {
		...
		if ( ! it_exchange_product_type_supports_feature( $product_type, 'membership-duration' ) )
			return;

		// If the value is empty (0), delete the key, otherwise save
		if ( empty( $_POST['it-exchange-product-membership-duration-time'] ) )
			delete_post_meta( $product_id, '_it-exchange-product-membership-duration-time' );
		else
			it_exchange_update_product_feature( $product_id, 'membership-duration', absint( $_POST['it-exchange-product-membership-duration-time'],  array( 'setting' => 'time' ) ) );

		it_exchange_update_product_feature( $product_id, 'membership-duration', $_POST['it-exchange-product-membership-duration-type'],  array( 'setting' => 'type' ) );
...
/**
	...
	*/
	function save_feature( $product_id, $new_value, $options=array() ) {
		...
		switch( $options['setting'] ) {
			...
			case 'type':
				return update_post_meta( $product_id, '_it-exchange-product-membership-duration-type', $new_value );
		
			case 'time':
			default:
				$new_value = empty( $new_value ) && !is_numeric( $new_value ) ? '' : absint( $new_value );
				return update_post_meta( $product_id, '_it-exchange-product-membership-duration-time', $new_value );
	...
	*/
	function get_feature( $existing, $product_id, $options=array() ) {
		...
		return get_post_meta( $product_id, '_it-exchange-product-membership-duration-' . $options['setting'], true );
	}
...
$IT_Exchange_Addon_Membership_Product_Feature_Duration = new IT_Exchange_Addon_Membership_Product_Feature_Duration();
